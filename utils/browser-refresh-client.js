...
exports.enableSpecialReload = function(patterns, options) {
    if (isBrowserRefreshEnabled() && process.send) {
        ...
        return {
            ...
            remove: function() {
                // Tim's hack: manually remove the listeners to cleanup between
                // Browser-Refresh child process (in Luncher) restart.
                specialReloadEvents.removeAllListeners();
                process.send({
...
function attachListeners() {
    ...
    // Tim's hack: best attempt to cleanup, not used atm.
    process.on('exit', () => {
